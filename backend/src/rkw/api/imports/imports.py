    ...
    if len(res) > 0:
        for row in res:
            ...
            if test_mode:
                ...
                sql = "INSERT INTO `api_ipg` (`Code`, `Description`, `EorH`) VALUES ('%s', '%s', '%s');" % (row[0], row[1], row[2])
            print(sql)
            mysql_cur.execute(sql)
            mysql_conn.commit()
